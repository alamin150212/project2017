...
Class RoutineDAO{
    ...
    public function getCountUser($day,$time,$userId){
        $SQL="SELECT count(user_id) AS c FROM rms_routine WHERE day_id='$day' and time_id='$time' and (another_user='$userId' or user_id='$userId')";
        $this->_DB->doQuery($SQL);
        $row = $this->_DB->getTopRow();
    ...
    public function getCountAnotherUser($day,$time,$userId){
        $SQL="SELECT count(another_user) AS c FROM rms_routine WHERE day_id='$day' and time_id='$time' and (another_user='$userId' or user_id='$userId')";
        $this->_DB->doQuery($SQL);
        $row = $this->_DB->getTopRow();

        $_SESSION["countAnotherUsers"] = $row['c'];
    ...
    public function getAllRoutineByDayYearTerm($Day,$Year,$Term){
        ...
        for($i = 0; $i < sizeof($rows); $i++) {
            ...
            $this->_Routine->setRoutineCourseId( $row['course_id'] );
            $this->_Routine->setRoutineUserId( $row['user_id'] );
            $this->_Routine->setRoutineAnotherUserId( $row['another_user'] );
            $this->_Routine->setRoutineDayId( $row['day_id'] );
            $this->_Routine->setRoutineTime( $row['time_id'] );
